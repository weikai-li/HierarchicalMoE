...
from kernel_runner import get_kernel_files
...
BENCHMARK = 'machsuite'
BENCHMARK = 'poly'
type_graph = 'multi_modality' ## 'programl'  ## 
...
MACHSUITE_KERNEL = ['aes', 'gemm-ncubed']
poly_KERNEL = [
    'symm-opt-medium', 'gemver-medium']
...
def get_tc_for_loop(for_loop_text):
    ...
    if delim:
        if delim == '--':
            TC = 0
            # comp = for_loop_text.split(';')[0].strip()
            # if ')' not in comp:
            #     raise NotImplementedError()
            # TC = int(eval((comp.replace(" ", "").split('=')[-1]).split(')')[-2].strip()))
            # print(TC, comp)
        else:
    ...
    if node_type == 'block':
        ...
        if connected:
            ...
            for idx, node in enumerate(sorted_nodes[:-1]):
                ...
                new_edges.append((id2, id1, e_dict))
                id += 1
                # print(f'edge between {node} with id {id1} and {sorted_nodes[idx+1]} and id {id2}')
...
def run_graph_gen(mode='initial', connected=True, target=['machsuite', 'poly'], ALL_KERNEL=ALL_KERNEL):
    ...
    # mode = 'initial'
    # mode = 'auxiliary' ## the previous mode must have been run before running this mode
    # mode = 'hierarchy' ## the two previous modes must have been run before running this mode
    # connected = True
    if mode == 'initial':
        if test != 'simple':
            for BENCHMARK in target:
                ...
                for kernel in ALL_KERNEL[BENCHMARK]:
                    ...
                    if not exists(path):
                        ...
                        programl_path = join(get_root_path(), f'programl/{BENCHMARK}/', f'{kernel}')
                        if exists(join(programl_path, f'{kernel}.c')):
                            ext ='c'
                            kernel_path = join(programl_path, f'{kernel}.c')
                        elif exists(join(programl_path, f'{kernel}.cpp')):
                            ext ='cpp'
                            kernel_path = join(programl_path, f'{kernel}.cpp')
                        else:
            ...
            processed_gexf_folder = join(get_root_path(), f'simple-program/{type_graph}/processed/initial')
            create_dir_if_not_exists(processed_gexf_folder)
            for kernel_name in target_kernel:
                if '1d-conv' in kernel_name:
                    continue
                for p in precision:
                    for s in size:
                        # current = f'dot-{p}-{s}'
                        path = join(f'/expr/simple-program/{kernel_name}')
                        if exists(path):
                            shutil.rmtree(path)
                        create_dir_if_not_exists(path)
                        src_path = join(get_root_path(), f'simple-program/sources/{kernel_name}/')
                        if not isfile(join(src_path, f'{kernel_name}-{p}-{s}_kernel.c')):
                            continue
                        shutil.copy(join(src_path, f'{kernel_name}-{p}-{s}_kernel.c'), 
                                    join(path, f'{kernel_name}-{p}-{s}.c'))
                        remove_extra_header(path, f'{kernel_name}-{p}-{s}')
                        print('####################')
                        print('now processing', kernel_name)
                        graph_generator(f'{kernel_name}-{p}-{s}', path, None, generate_programl = True)
                        print()
    elif mode == 'auxiliary':
        ...
        for BENCHMARK in target:
            if BENCHMARK != 'simple':
                ...
                processed_gexf_folder = join(get_root_path(), f'simple-program/{type_graph}/processed/initial')
                if connected:
                    auxiliary_node_gexf_folder = join(get_root_path(), f'simple-program/{type_graph}/processed/extended-pseudo-block-connected/')
                else:
                    auxiliary_node_gexf_folder = join(get_root_path(), f'simple-program/{type_graph}/processed/extended-pseudo-block-base/')
                create_dir_if_not_exists(auxiliary_node_gexf_folder)
                all_kernel = []
                kernel_root_path = join(dirname(dirname(get_root_path())), 'lorenzo-data/selected-kernels')
                kernel_info = get_kernel_files(kernel_root_path)
                ...
                for kernel_name in target_kernel:
                    ...
                    for p in precision:
                        for s in size:
                            all_kernel.append(f'{kernel_name}-{p}-{s}')
            for kernel in all_kernel:
                if False and 'aes' not in kernel:
                    continue
                print('####################')
                print('now processing', kernel)
        ...
        # write_csv_file(csv_dict, csv_header, f'programl/hierarchy.csv', csv_dict=csv_dict)
